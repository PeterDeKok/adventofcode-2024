...
func Solution(_ context.Context, _ *logger.IterationLogger, rd io.Reader, w io.Writer) error {
	...
	for i, line := range input.LineIsIntSliceReader(rd) {
		...
		if build.DEBUG {
			if err != nil {
				fmt.Printf("[line %d | %v]: %v | %v\n", i, line, lineResult, err)
			} else {
				fmt.Printf("[line %d | %v]\n", i, line)
			}
		}
		...
		for j := 0; j < len(line); j++ {
			filtered := make([]int, len(line)-1)
			copy(filtered[:j], line[:j])
			copy(filtered[j:], line[j+1:])

			lineResult, ok, err = handleLine(filtered)
			if !ok && err != nil {
				panic(fmt.Errorf("failed to handle line %d: %v", i, err))
			} else if !ok {
				panic(fmt.Errorf("failed to handle line %d: unknown error", i))
			...
			if build.DEBUG {
				if err != nil {
					fmt.Printf("  [line %d | f: %d | %v]: %v | %v\n", i, j, filtered, lineResult, err)
				} else {
					fmt.Printf("  [line %d | f: %d | %v]: %v\n", i, j, filtered, lineResult)
				}
			}
